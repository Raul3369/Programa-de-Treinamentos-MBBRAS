...
def window2(ID,section):
    # Outputs da tela

    output = sg.Text(font=("Arial",20),key="output",visible=False) 
    output2 = sg.Text(font=("Arial",20),key="output2",visible=False)
    ...
    tutorials = {filename.split('.')[0]:filename for filename in already_watched(ID).tolist()}
